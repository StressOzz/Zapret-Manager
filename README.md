...
- Станут работать многие сайты, а так же **rutor.info**, **Instagram*** на ПК и в приложении, **ntc.party**, **www.lib.rus.ec**...
- Добавлен обход запрещённых тем на **4Pda.to** например таких как:
[Безопасность в сети](https://4pda.to/forum/index.php?showforum=1192) или [Способы обходов блокировок на прошивке OpenWrt](https://4pda.to/forum/index.php?showtopic=1085698) на всех стратегиях...
- Заработают сетевые режимы в играх, включая **BF 6**, **Apex Legend**, **Roblox** и в других...

> [!IMPORTANT]
> Весь трафик пойдёт через Zapret !!!

пункт 0 - **Установка и настройка DNS over HTTP**
- Пункт активный, т.е. если уже установлен DNS over HTTP, то появится надпись перед меню и пункт 0 будет называться - Удалить DNS over HTTP.
Если DNS over HTTP не установлен, то пункт будет называться - Установить и настроить DNS over HTTPS.
Настройка заключается в установки **Comss.one DNS**. После установки над меню появится надпись, что DNS over HTTP установлен и настроен...
