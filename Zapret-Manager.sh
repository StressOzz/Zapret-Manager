...
if [ -f /etc/init.d/zapret ] && [ -f "$CONF" ]; then if grep -Eq "^[[:space:]]*option DISABLE_IPV6 '1'" "$CONF"; then echo -e "${CYAN}7) ${GREEN}Включить ${NC}IPv6${GREEN} в ${NC}Zapret"; elif grep -Eq "^[[:space:]]*option DISABLE_IPV6 '0'" "$CONF"; then echo -e "${CYAN}7) ${GREEN}Выключить ${NC}IPv6${GREEN} в ${NC}Zapret"; fi; fi
